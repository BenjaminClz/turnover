...
const emptyDetailsForm = { date_naissance: '', taille_cm: '', poids_kg: '', annees_pratique: '', clubs_precedents: '', bio: '' };
...
export default function PlayersTab({ user, profile, showToast, onContact, onViewGallery }) {
  ...
  const load = async () => {
    ...
    if (mine) {
      ...
      setDetailsForm({
        ...
        annees_pratique: mine.annees_pratique ?? '', clubs_precedents: mine.clubs_precedents || '', bio: mine.bio || '',
      });
    }
    setLoading(false);
  };
  ...
  const handleSaveDetails = async (e) => {
    ...
    const payload = {
      ...
      clubs_precedents: detailsForm.clubs_precedents || null,
    ...
    <div>
      ...
      {profile.role === 'joueur' && myListing && editingDetails && (
        <div style={{ background: '#152E26', border: '1.5px solid #2C4A3D', borderRadius: 18, padding: 28, marginBottom: 36 }}>
          ...
          <form onSubmit={handleSaveDetails}>
            ...
            <Field label="Clubs précédents (facultatif)"><TextInput value={detailsForm.clubs_precedents} onChange={(e) => setDetailsForm({ ...detailsForm, clubs_precedents: e.target.value })} placeholder="ex. RC Annemasse (2020-2023), CO Genève (2018-2020)" /></Field>
      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gap: 12 }}>
          {players.map((p) => (
            <div key={p.id} onClick={() => setViewingPlayer(p)} style={{ background: '#152E26', border: '1.5px solid #2C4A3D', borderRadius: 14, padding: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16, flexWrap: 'wrap', cursor: 'pointer' }}>
              <div style={{ display: 'flex', gap: 16 }}>
                ...
                <div>
                  ...
                  {p.clubs_precedents && <div style={{ fontSize: 13, color: '#C7CFC8', marginTop: 4 }}>Clubs précédents : {p.clubs_precedents}</div>}
