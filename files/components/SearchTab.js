            ...
            <div>
              ...
              {filteredNeeds.length === 0 ? <EmptyState icon="📋" title="Rien ici" sub="Aucun résultat pour ces critères." /> : (
                <div style={{ display: 'grid', gap: 10 }}>
                  {filteredNeeds.map((n) => (
                    <ResultRow key={n.id} title={n.club} details={`${needDetails(n)} · ${n.ville}`} distance={n._distance} showContact={n.owner_id !== user.id} onContact={() => onContact(n.owner_id, n.club, needDetails(n))} avatarPath={n.profiles?.avatar_path} supabase={supabase} extra={<GhostButton onClick={() => onViewGallery(n.owner_id, n.club)} style={{ fontSize: 12 }}>Voir le club</GhostButton>} />
                  ))}
                </div>
              )}
            </div>
            ...
            return (
              <div key={role}>
                ...
                {list.length === 0 ? <EmptyState icon="🧑‍⚕️" title="Rien ici" sub="Aucun résultat pour ces critères." /> : (
                  <div style={{ display: 'grid', gap: 10 }}>
                    {list.map((s) => (
                      <ResultRow key={s.id} title={s.profiles?.nom} details={`${staffDetails(s)} · ${s.ville}`} distance={s._distance} showContact={s.owner_id !== user.id} onContact={() => onContact(s.owner_id, s.profiles?.nom, staffDetails(s))} avatarPath={s.profiles?.avatar_path} supabase={supabase} />
                    ))}
                  </div>
                )}
              </div>
...
function ResultRow({ title, details, distance, showContact, onContact, extra, avatarPath, supabase }) {
