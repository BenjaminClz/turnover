            ...
            <div>
              ...
              {filteredNeeds.length === 0 ? <EmptyState icon="📋" title="Rien ici" sub="Aucun résultat pour ces critères." /> : (
                <div style={{ display: 'grid', gap: 10 }}>
                  {filteredNeeds.map((n) => (
                    <ResultRow key={n.id} title={n.club} details={`${needDetails(n)} · ${n.ville}`} distance={n._distance} showContact={n.owner_id !== user.id} onContact={() => onContact(n.owner_id, n.club, needDetails(n))} avatarPath={n.profiles?.avatar_path} supabase={supabase} featured={n._featured} extra={<GhostButton onClick={() => onViewGallery(n.owner_id, n.club)} style={{ fontSize: 12 }}>Voir le club</GhostButton>} reportProps={{ targetType: 'club_need', targetId: n.id, targetOwnerId: n.owner_id, reporterId: user.id, showToast }} favoriteProps={{ targetType: 'club_need', targetId: n.id, ownerId: user.id }} />
                  ))}
                </div>
              )}
            </div>
              ...
              {filteredPlayers.length === 0 ? <EmptyState icon="👤" title="Rien ici" sub="Aucun résultat pour ces critères." /> : (
                <div style={{ display: 'grid', gap: 10 }}>
                  {filteredPlayers.map((p) => (
                    <ResultRow key={p.id} title={p.profiles?.nom} details={`${p.poste} · ${p.niveau} · ${p.ville}`} distance={p._distance} showContact={p.owner_id !== user.id} onContact={() => onContact(p.owner_id, p.profiles?.nom, `${p.poste} · ${p.ville}`)} avatarPath={p.profiles?.avatar_path} supabase={supabase} onClick={() => setViewingPlayer(p)} extra={<GhostButton onClick={(e) => { e.stopPropagation(); onViewGallery(p.owner_id, p.profiles?.nom); }} style={{ fontSize: 12 }}>Galerie</GhostButton>} reportProps={{ targetType: 'player_listing', targetId: p.id, targetOwnerId: p.owner_id, reporterId: user.id, showToast }} favoriteProps={{ targetType: 'player_listing', targetId: p.id, ownerId: user.id }} />
                  ))}
                </div>
              )}
            </div>
...
function ResultRow({ title, details, distance, showContact, onContact, extra, avatarPath, supabase, featured, onClick, reportProps, favoriteProps }) {
  ...
  return (
    <div onClick={onClick} style={{ background: featured ? 'rgba(212,255,63,0.05)' : '#152E26', border: featured ? '1.5px solid #D4FF3F' : '1.5px solid #2C4A3D', borderRadius: 12, padding: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap', cursor: onClick ? 'pointer' : 'default' }}>
